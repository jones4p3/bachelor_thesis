...
def create_file_paths_list(radar, data, endswith=".nc"):
    ...
    logger.debug(f"Data path for radar {radar.slug}: {data_path}")
...
def adding_instrument_height(radar, ds):
    if radar.add_to_range is False:
        logger.debug(f"No instrument height addition specified for radar {radar.slug}")
...
def drop_variables(radar, ds):
    ...
    if not vars_to_keep:
        ...
        logger.debug(f"Keeping specified variables for radar {radar.slug}: {vars_to_keep}")
        vars_in_ds = list(ds.data_vars)
        vars_to_drop = [var for var in vars_in_ds if var not in vars_to_keep]
        ds = ds.drop_vars(vars_to_drop)
        logger.debug(f"Dropped unnecessary variables for radar {radar.slug}")
    return ds
...
def check_time_dimension(radar, ds):
    ...
    if ds[time_dim].dtype != "datetime64[s]":
        ...
        logger.debug(f"Converted time dimension to datetime64[s] for radar {radar.slug}")
    ...
    if not is_time_index_unique:
        ...
        logger.debug(f"Dropped duplicate time indices for radar {radar.slug}")
    return ds
...
def convert_linear_to_dBZ(radar, ds):
    ...
    if not convertion_needed:
        logger.debug(f"No conversion from mm6 to dBZ needed for radar {radar.slug}")
    else:
        ...
        logger.debug(f"Converted {ze_var_name} from mm6 to dBZ for radar {radar.slug}")
    return ds
...
def add_attributes(radar, ds):
    ...
    if radar_attributes:
        for attr_key, attr_value in radar_attributes.items():
            ...
            logger.debug(f"Added attribute {attr_key}: {attr_value} to dataset for radar {radar.slug}")
        logger.debug(f"Added attributes to dataset for radar {radar.slug}")
    return ds
...
def sanitize_attributes(radar, ds):
    for attr_key, attr_value in ds.attrs.items():
        ...
        if not is_clean:
            ...
            ds.attrs[attr_key] = attr_value
            logger.debug(f"Sanitized attribute {attr_key} for radar {radar.slug}")
    logger.debug(f"Checked sanitization attributes for radar {radar.slug}")
    return ds