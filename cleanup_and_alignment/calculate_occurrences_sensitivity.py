...
def calculate_occurrences(ds, bin_edges, use_aligned=False):
    ...
    counts_for_each_height = np.zeros(
        (nheight, nbins), dtype=int
    )  # Array with shape (nheight, nbins)
    ...
    for current_height in range(nheight):
        ...
        counts_at_current_height, _ = np.histogram(
            ze_at_height, bins=bin_edges
        )  # counts_at_current_height contains the counts for each bin shape: (nbins,)
    ...
    occurrences = (
        counts_for_each_height / n_measurements
    ) * 100  # Convert occurrences to percentage
    ...
    xr_da_occurrences["height"].attrs.update(
        long_name="Height above ground level", units="m"
    )
    xr_da_occurrences[bin_name].attrs.update(long_name="Reflectivity", units="dBZ")
    ds[xr_da_occurrences.name] = (
        xr_da_occurrences  # Add DataArray to the original dataset
    )
...
def calculate_sensitivity(ds, threshold=0.999, min_samples_per_height=50):
    ...
    n_samples = ds["counts"].sum(
        dim="bin"
    )  # Total number of samples for each height level
    sensitvity = sensitvity.where(n_samples >= min_samples_per_height, np.nan)
...
def calculate_occurrences_and_sensitivity_for_all_radars(data, params):
    # Creating global bin edges
    ...
    for radar_handle, ds in data.radar_datasets.items():
        ...
        data.radar_datasets[radar_handle] = calculate_sensitivity(
            data.radar_datasets[radar_handle],
